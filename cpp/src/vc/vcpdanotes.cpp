...
// VCPDANotes — byte-exact Read/Write per W23.4 (base init + sub-coll tag 'f').
#include "vc/vcpdanotes.h"
...
VCPDANotes::VCPDANotes() noexcept : _npfl_flags(0), _vers(5) {
...
VCPDANotes::~VCPDANotes() = default;

void VCPDANotes::Read(HDBContext* ctx, uint32_t /*param_2*/) {
    if (!ctx) return;
    ctx->read_base_init(_npfl_flags, _vers);
    sub_coll.read_header(ctx, 0x66);
}

void VCPDANotes::Write(HDBContext* ctx) const {
    if (!ctx) return;
