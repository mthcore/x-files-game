...
// VCTriggerList.cpp - byte-direct Read (slot 12) decoder.
...
VCTriggerList::VCTriggerList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCTriggerList::~VCTriggerList() = default;

void VCTriggerList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCTriggerList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
