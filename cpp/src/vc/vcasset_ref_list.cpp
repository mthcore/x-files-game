...
// VCAssetRefList.cpp - byte-direct Read (slot 12) decoder.
...
VCAssetRefList::VCAssetRefList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCAssetRefList::~VCAssetRefList() = default;

void VCAssetRefList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCAssetRefList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
