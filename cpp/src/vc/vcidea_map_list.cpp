...
// VCIdeaMapList.cpp - byte-direct Read (slot 12) decoder.
...
VCIdeaMapList::VCIdeaMapList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCIdeaMapList::~VCIdeaMapList() = default;

void VCIdeaMapList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCIdeaMapList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
