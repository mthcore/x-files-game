...
// VCExplorableList.cpp - byte-direct Read (slot 12) decoder.
...
VCExplorableList::VCExplorableList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCExplorableList::~VCExplorableList() = default;

void VCExplorableList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCExplorableList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
