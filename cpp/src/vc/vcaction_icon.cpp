...
// VCActionIcon.cpp - byte-direct Read (slot 12) decoder; thunks to the shared
// Icon-family reader. Body identical for all 5 Icon family classes.

#include "vc/vcaction_icon.h"
...
VCActionIcon::VCActionIcon() noexcept
    : cache_e(0), cache_f(0), cache_g(0), cache_h(0),
      cache_k(0), cache_i(0), cache_j(0) {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
}

VCActionIcon::~VCActionIcon() = default;

void VCActionIcon::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCActionIcon::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
