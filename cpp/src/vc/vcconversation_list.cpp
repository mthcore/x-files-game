...
// VCConversationList.cpp - byte-direct Read (slot 12) decoder.
...
VCConversationList::VCConversationList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCConversationList::~VCConversationList() = default;

void VCConversationList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCConversationList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
