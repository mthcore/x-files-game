...
// VCNavList.cpp - byte-direct Read (slot 12) decoder.
...
VCNavList::VCNavList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCNavList::~VCNavList() = default;

void VCNavList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCNavList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
