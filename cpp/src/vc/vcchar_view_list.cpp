...
// VCCharViewList.cpp - byte-direct Read (slot 12) decoder.
...
VCCharViewList::VCCharViewList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCCharViewList::~VCCharViewList() = default;

void VCCharViewList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCCharViewList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
