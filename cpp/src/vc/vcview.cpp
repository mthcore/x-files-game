...
// VCView.cpp — byte-direct Read (slot 12) decoder.
...
VCView::VCView() noexcept
    : cache_e(0), cache_f(0), cache_g(0), cache_h(0),
      cache_i(0), cache_j(0), field_40(0) {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
}

VCView::~VCView() = default;

void VCView::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCView::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
