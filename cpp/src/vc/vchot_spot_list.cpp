...
// VCHotSpotList.cpp - byte-direct Read (slot 12) decoder.
...
VCHotSpotList::VCHotSpotList() noexcept {
    std::memset(_base_scratch, 0, sizeof(_base_scratch));
    std::memset(sub_coll,      0, sizeof(sub_coll));
}

VCHotSpotList::~VCHotSpotList() = default;

void VCHotSpotList::Read(hdb::HDBContext* ctx, uint32_t /*mode*/) {
...
void VCHotSpotList::Write(hdb::HDBContext* ctx) const {
    if (ctx == nullptr) return;
