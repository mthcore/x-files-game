// SPDX-License-Identifier: MIT
// VCEvidenceIcon.cpp — identical TLV grammar to VCInventory (shared).

#include "vc/vcevidence_icon.h"
#include "hdb/hdb_context.h"
#include "hdb/hdb_tlv.h"

#include <cstring>

namespace xfiles {
namespace vc {

VCEvidenceIcon::VCEvidenceIcon() noexcept
    : _npfl_flags(0), _vers(5),
      field_e(0), field_f(0), field_g(0), field_h(0),
      field_k(0), field_i(0), field_j(0) {
    std::memset(_engine_internal_0, 0, sizeof(_engine_internal_0));
    std::memset(_intermediate_0, 0, sizeof(_intermediate_0));
}

VCEvidenceIcon::~VCEvidenceIcon() = default;

void VCEvidenceIcon::Read(HDBContext* ctx, uint32_t /*param_2*/) {
    if (!ctx) return;
    ctx->read_base_init(_npfl_flags, _vers);
    auto& r = ctx->reader();
    field_e = r.read_uint32_field(0x65);
    field_f = r.read_uint32_field(0x66);
    field_g = r.read_uint32_field(0x67);
    field_h = r.read_uint32_field(0x68);
    field_i = r.read_uint32_field(0x69);
    field_j = r.read_uint32_field(0x6a);
    if (_vers > 3) field_k = r.read_uint32_field(0x6b);
}

void VCEvidenceIcon::Write(HDBContext* ctx) const {
    if (!ctx) return;
    ctx->write_base_init(_npfl_flags, _vers);
    auto& w = ctx->writer();
    w.write_uint32_field(0x65, field_e);
    w.write_uint32_field(0x66, field_f);
    w.write_uint32_field(0x67, field_g);
    w.write_uint32_field(0x68, field_h);
    w.write_uint32_field(0x69, field_i);
    w.write_uint32_field(0x6a, field_j);
    if (_vers > 3) w.write_uint32_field(0x6b, field_k);
}

}  // namespace vc
}  // namespace xfiles
